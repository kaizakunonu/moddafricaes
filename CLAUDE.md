...
- `transform_era5land_minmax(input_dir, output_dir, years, ...)` — reduces the multi-variable hourly NetCDFs `extract_era5land()` produced to monthly min and monthly max 2 m temperature (degC) GeoTIFFs. Selects only `t2m` layers via the `t2m_valid_time=<unix>` naming convention, caches one TIFF per (year, month), and writes a 12-layer annual stack once a year is complete.
